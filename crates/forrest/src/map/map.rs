...
///   2. Zero, one or two hash values.
///
/// For example: `H(prefix || hash0 || hash1)`.
///
/// ### Leaf Nodes
///
...
impl<D: Digest, K: AsRef<[u8]>, V: AsRef<[u8]>> Map<D, K, V> {
    ...
    pub fn insert(&self, key: K, val: V) -> Self {
        let (node, new) = self.0.node.insert(Path::from(&key), (key, val));
        Self(node.into(), self.1 + usize::from(new))
    }

    /// Inserts all key/value pairs into the map, creating a new map.
    pub fn extend(&self, iter: impl IntoIterator<Item = (K, V)>) -> Self {
        let mut node = self.0.node.clone();
        let mut size = self.1;

        for (key, val) in iter {
            let (val, new) = node.insert(Path::from(&key), (key, val));
            size += usize::from(new);
            node = val;
        }
