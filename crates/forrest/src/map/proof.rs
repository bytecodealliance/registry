...
impl<D: Digest, H: Deref<Target = Hash<D>>, V: AsRef<[u8]>> Proof<D, H, V> {
    ...
    pub fn verify<Q: ?Sized + AsRef<[u8]>>(&self, root: &Hash<D>, key: &Q) -> bool {
        ...
        let mut hash = D::new()
            .chain_update(&[0xff])
            .chain_update(D::digest(key))
            .chain_update(D::digest(self.value.as_ref()))
            .finalize()
            .into();

        // Loop over each side and peer.
        for (side, peer) in path.zip(fill.chain(self.peers.iter().map(|x| x.as_deref()))) {
